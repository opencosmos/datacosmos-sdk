...
class Config(BaseSettings):
    ...
    def settings_customise_sources(cls, *args, **kwargs):
        init_settings = kwargs.get("init_settings") or (args[1] if len(args) > 1 else None)
        env_settings = kwargs.get("env_settings") or (args[2] if len(args) > 2 else None)
        dotenv_settings = kwargs.get("dotenv_settings") or (args[3] if len(args) > 3 else None)
        file_secret_settings = kwargs.get("file_secret_settings") or (args[4] if len(args) > 4 else None)
    ...
    def _parse_authentication(cls, raw):
        ...
        from datacosmos.config.models.local_user_account_authentication_config import LocalUserAccountAuthenticationConfig
