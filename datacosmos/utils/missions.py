from datacosmos.utils.constants import TEST_MISSION_NAMES, PROD_MISSION_NAMES
...
def get_mission_id(mission_name: str, env: str) -> int:
    ...
    if env == "test" or env == "local":
        return {v.upper(): k for k, v in TEST_MISSION_NAMES.items()}[mission_name.upper()]
    elif env == "prod":
        return {v.upper(): k for k, v in PROD_MISSION_NAMES.items()}[mission_name.upper()]
    else:
        raise ValueError(f"Unsupported environment: {env}")